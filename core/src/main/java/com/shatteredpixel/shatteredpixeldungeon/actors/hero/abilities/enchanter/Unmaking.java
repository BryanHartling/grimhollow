...
public class Unmaking extends ArmorAbility {
    ...
    public void unmake(Char enemy,Hero hero){int removed=EnchanterMagic.strip(enemy),turns=Char.hasProp(enemy,Char.Property.BOSS)?4:8;Buff.prolong(enemy,Unmade.class,turns);Buff.prolong(enemy,Vulnerable.class,turns);Buff.prolong(enemy,Cripple.class,turns);Buff.prolong(enemy,EnchanterMagic.EnchanterDamage.class,turns);hero.HP=Math.min(hero.HT,hero.HP+removed*hero.pointsInTalent(Talent.RECLAMATION));EnchanterMagic.counterweight();}
