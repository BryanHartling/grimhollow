...
- Add Hatchling Mimic to the unchanged-weight trinket pool, using the existing carried-trinket/catalyst recipe conventions. Its three cauldron upgrades cost 10/15/20 energy. Hunger, the one-turn warning, permanently doubling gold demand and floor charm history serialize with the item, including cauldron duplication and dropping/recovery. The clock pauses while the trinket is not carried.
- Feed only the agreed loose item families; protect equipped gear, bags and contents, class focuses, quest items, Waterskin and all unlisted belongings. Low-tier thrown stacks are eaten together for one benefit; seeds, stones, potions and scrolls lose one unit. Identified food is preferred within each priority category. Benefits independently choose the least valuable eligible loose target, breaking ties randomly; this allows deliberate steering toward permanent upgrades. Consumables can be identified.
- Use the agreed gold tier table through 3200+ Exceptional. Gold demand never resets after an item meal, upgrade, save/load or floor transition. Warnings interrupt resting and travel. If every spawn cell is occupied, an artifact meal remains warned and is postponed until a valid cell exists.
- Every transformed/escaped mimic receives normal loot plus the actual +10 Ring of Wealth equipment drop: eligible equipment is uncursed and upgradable gear has at least +5. The shared generator naturally includes its eligible Grimhollow additions. The consumed artifact type is excluded; a Hatchling or catalyst cannot come from that equipment pool. Artifact-to-Crystal probability uses the artifact's normalized 0-10 level, at every depth.
- Ordinary and Golden Mimics retreat; one ordinary mimic per floor can use the Siren's Song ally conversion until attacked or left behind. Ebony Mimics wake on room entry. Crystal Mimics pursue on room entry, contest theft against evasion with the ordinary Thief's accuracy of 12, then run toward the deeper exit; they escape only there, out of sight and at least six cells away. Killing one before escape returns the original trinket with its hunger/debt, without the extra reward. Standard turn scheduling remains intact.
- Escaped mimics appear on the next non-boss floor; at floor 24 that is 26. With no deeper floor beyond 26, the encounter waits on floor 24 during the return ascent. Crystal theft preserves the fleeing Crystal type. No new floor or special Playtest interaction is introduced.
- Item Sense draws object markers after fog without revealing surrounding terrain or ordinary enemies. Repeated meals refresh their detection duration rather than stack it; lesser meals do not cancel remaining exceptional mimic detection. Detection expires on leaving the floor.
- Replace player-facing Shattered Pixel Dungeon branding with Grimhollow while retaining copyright, attribution and credits. Replace the six region introductions and thirty regional journal pages with an original linked Grimhollow story. Untranslated narrative falls back to the new English text rather than the superseded story.
- Commit five original painted region backgrounds and the Hatchling icon, their exact imagegen prompts, and offline atlas packaging. Raised skeletons are repacked from the existing original painting at double the prior resolution, retaining world size and animation timing. No runtime generation is required.
- Package a native Grimhollow.exe with the painted Windows icon and bundled runtime; the launcher prefers this over javaw. Disable GLFW's automatic fullscreen iconification on focus loss. The existing shortcut remains the stable launch entry point.
- Prevent unknown neighboring terrain from influencing visible wall caps and wall-face stitching. Refresh newly discovered neighborhoods as observation changes, while preserving known walls in remembered terrain. Visible loot receives a subtle post-gas draw pass before fog; unseen loot remains hidden unless an item-detection effect explicitly reveals it.
- Expand long titled descriptions to the available viewport, preserving scrolling through the last line. Focus Crystal Grasp opens bones/remains at level 3 and ordinary unlocked chests at level 7 for its existing charge/turn cost; collecting the exposed loot takes a separate cast. Locks remain, haunted remains retain their consequences, and remote opening can wake mimics.
- Persist journal reference discoveries across games, including discoveries made while playtesting. Per-run potion/scroll/ring identity mappings still reset; Playtest badge, ranking, usage-credit and bones exclusions remain.

# Playtest, movement and readability fixes - v1.12.0

- Inscribe now opens an equipment picker that includes carried thrown weapons and the Spirit Bow. Ranged choices use the identified/starting library minus the explicit melee-contact-only metadata flag. Every currently shipping enchantment has a meaningful ranged effect; none is arbitrarily excluded. Thrown stack splitting/merging preserves the inscription and its remaining duration.
- Resolve Lucky once per hit across inscription, Rune Etching, permanent enchantment and shared bow effects: a failed later roll cannot erase an earlier success. A later attack clears the previous hit's result. Overcharge works with either Lucky slot. Lucky remains a killing-hit effect, and upstream's no-loot rule for enemies more than two levels below the hero's loot eligibility remains; its description now explains both conditions.
- Reduce movement-triggered light-map samples and uploads sixteenfold (four samples per tile edge instead of sixteen), bulk-write pixels and update the existing GPU texture. Linear sampling retains the light field; a numerical regression bounds the difference to ten channel values out of 255. This targets the reported Samsung SM-T830 movement stutter without changing turn costs, pathfinding or the existing Android movement duration.
- Hold non-hero resting/travel poses steady while position tweening continues; attack, spell and death callbacks retain their timing. Pack Brute and all Shaman variants directly from the existing source paintings at twice their former atlas resolution, preserving world size and avoiding repeated lossy downsampling.
- Skill details show all rank effects together in the scrolling description, without rank-toggle buttons. Purchasing still enforces the three/four-rank limits.
- Replace the sewer wall decoration's obsolete leaking-pipe emitter with the existing torch emitter. The old water drips were a leftover from upstream's pipe art, not a new torch mechanic.
- Replace the unsolicited Worn Key GitHub prompt with an explicit in-game Report an issue action. Both this and error dialogs show scrollable diagnostic text with Copy report, Close and optional Open GitHub; reports are not sent automatically.
- Reuse a single starter Velvet Pouch across Playtest class switches. Merge duplicate bags on load/class switch, preserving their contents and quickslot reference, including overflow contents from old saves.
- Keep the cap of a known wall when its adjoining floor becomes remembered; reject overhangs from unknown source cells. Test 43 now requires these real remembered boundaries instead of requiring every neighboring wall cap to disappear; strict unseen-cell blackness and fog alignment in test 47 are unchanged.
- Replace all 60 item-identification mini-icons with semantically mapped crops of existing painted status sources (Strength uses a fist, Mind Vision an eye). Inventory quantities, strengths and upgrade numbers use larger screen-resolution text; the offline packer and per-index semantic contract reproduce the new atlas.
- Reject solid/out-of-map arrival positions and fall back to a valid entrance/transition when an unvisited boss floor still has a placeholder exit. Exercise actual ascent from floors 6/11/16/21/26 after Playtest travel. Mining's intentional solid-cell exception remains.
- Tengu's smoke/fuses use soft painted mist and his shockers use cyan sparks and smooth glowing arcs. Enhanced Effects off retains upstream particles. Damage cells, hazard timing and combat mechanics are unchanged.
...
- Let the Focus Crystal and Sigil Brush use upstream artifact unequip/re-equip. Their level, experience and stored charges persist; a bagged focus cannot cast or passively recharge. Drop/throw protection remains.
- Android visual steps take 1.8 times their previous duration (ordinary step 0.10 -> 0.18 seconds); hero running uses 12 instead of 20 frames/second and gentler camera follow. Exponential camera smoothing is frame-rate independent. Actor turn costs and desktop step timing are unchanged.
- An overhang may draw only when its source cell below is visible, visited or mapped. This hides unknown wall/door/prop tops without erasing remembered terrain. Existing fog scale remains untouched.
- Render the existing painted barricade boards across the passage, fill the span, and draw them above adjoining wall caps but below fog. Atlas files and collision rules are unchanged.
- Forward completed taps from ordinary scroll panes to nested buttons, preserving drag cancellation. This repairs the character handbook's talents and armor choices. Talent windows have rank-preview buttons, 214 distinct added-class rank descriptions, and a scrolling body. Titled lore/skill/buff windows also scroll; their text cameras follow resizing and offsets.
- Keep three armor paths per class, as requested. All nine added-class paths now have four talents with four ranks. Existing upstream paths already satisfy this contract. Armor charge discounts remain the fourth talent, Heroic Energy.
- Give every previously repeated added-class rank a benefit: Focused Mind +1/2/3 capacity; Far Reach +1/2/4 Grasp cells; Precognition one/two threshold-hit preventions per floor with save migration; Treasure Sense loot/doors/traps by rank; Shared Will Bless/Haste/Barkskin by rank; Grave Wisdom 1.25/1.5/1.75 experience and 2/4/6-turn Wither Cripple; Deep Knowledge 1/2/3 discoveries per new floor; Wide Field +1/2/3 radius.
- Armor ranks: Wider Blast +1/2/3/4 radius; Soul Refund/Feedback/Cascade/Mobile 25/50/75/100 percent; Kinetic Surge +12.5/25/37.5/50 percent; Backlash one charge per 4/3/2/1 affected enemies; Total Sight hidden-door radius 3/6/9 then full-floor secrets/map; Permeable throws through walls at rank 1, +10/20/30 percent thrown accuracy at ranks 2/3/4, and hero passage at ranks 3/4. Other linear armor effects extend to four ranks. These numeric rebalance choices replace the previous duplicate or zero-benefit ranks.
- Infernal Brew supplies three Ashlight feeding units, not three unconditional high-level upgrades. Liquid Flame/Dragon's Breath/Soulfire values and the fourteen-unit cap curve remain unchanged. Level-8 Flare includes disguised neutral mimics, reveals them via their normal negative-buff hook, and spares allied mimics. No ambient lantern burn or holy damage was added.
- Upgrade scrolls use two rising/expanding gold layers from the existing painted inscription effect. The first frame is initialized before draw; no new art source or generation is involved.
- Extend the existing smoke/native harness with test 53: equipment swap, feeding, mimic ignition thresholds, rank effects/text, armor contracts, mobile timing, real touch handbook/rank/scroll/Hurl input and unknown overhangs. Test expectations for maximum-rank Bone Prison and Amplified now assert the new fourth-rank effects.
...
- Batch equal-alpha additive flame/ember sprites while retaining every particle and its motion; an in-renderer comparison checks the individual-draw reference. Halving gas alone still missed Linux p95 (2.5816ms); the unchanged effects gate covers both gas and fire.
