...
public class PsychicStorm extends ArmorAbility {
    ...
    @Override public void activate(ClassArmor armor,Hero hero,Integer cell){if(armor.charge<chargeUse(hero))return;int affected=0;
        ...
        int p=hero.pointsInTalent(Talent.BACKLASH);FocusCrystal crystal=hero.belongings.getItem(FocusCrystal.class);if(p>0&&crystal!=null)crystal.gainCharge(affected/(5-p));armor.charge-=chargeUse(hero);com.shatteredpixel.shatteredpixeldungeon.items.Item.updateQuickslot();hero.spendAndNext(1);
    }
