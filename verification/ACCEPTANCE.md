...
Source commits: `9db9fe525` (wand/text correction) and `99a1f4371` (painted creatures, status icons, display scale, torch materials and verification). Delivery adds current evidence and documentation. Local tag: `v1.4.0-painted-bestiary`. Publication was attempted but the supplied GitHub credential returned HTTP 401; no saved credential or SSH configuration is available. The remote remains `e91123eb4ca41b20fb48a4e8985acfd368199b49`. Delivered-head CI was NOT RUN; no remote release/tag is claimed.
...
| 21 | permanent known issue | Unchanged test 45 fails 26/82 locally. Desktop and Android packages build; delivered-head CI NOT RUN because GitHub publication returned HTTP 401. |
